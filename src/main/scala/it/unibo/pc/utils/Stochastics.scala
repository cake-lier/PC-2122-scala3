...
object Stochastics extends App {
  ...
  def uniformDraw[A](actions: Set[A])(using rnd: Random = random): A = actions.toList(random.nextInt(actions.size))

  def drawFiltered(filter: Double => Boolean)(using rnd: Random = random): Boolean = filter(rnd.nextDouble())
